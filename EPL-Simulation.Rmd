...
for (i in 1:iSimulation){
  ...
  tmp <- EPL.new %>% 
              mutate(x1 = rpois(nrow(EPL.new), lambda = EPL.new$GS.by.H), 
                     x2 = rpois(nrow(EPL.new), lambda = EPL.new$GS.by.A), 
...
## Winner
```{r winner model}
EPL.all.new %>% filter(Rank == 5) %>% select(Team) %>% table/iSimulation
```
